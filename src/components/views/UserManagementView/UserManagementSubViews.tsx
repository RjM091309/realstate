  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((r) => {
            const extra = Math.max(0, r.userCount - r.assignedUsers.length);
            const canRemove = r.id !== ADMIN_ROLE_ID && r.userCount === 0 && r.active;
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      <h2 className="text-lg font-semibold leading-tight text-slate-900 dark:text-slate-50">{r.name}</h2>
                ...
                <div className="mt-4 flex min-h-[40px] flex-wrap items-center gap-1">
                  {r.assignedUsers.map((u) => (
                    <div
                      key={u.id}
                      title={[u.firstName, u.lastName].filter(Boolean).join(' ') || u.username}
                      className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full border border-slate-200 bg-slate-100 text-[10px] font-bold text-slate-700 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200"
                    >
                      {u.avatarUrl ? (
                  ...
                  {extra > 0 ? (
                    <span className="flex h-9 min-w-9 items-center justify-center rounded-full bg-slate-200 px-2 text-xs font-semibold text-slate-700 dark:bg-slate-700 dark:text-slate-200">
                      {t('views.userRole.moreUsers', { count: extra })}
                    </span>
                  ) : null}
                  {r.userCount === 0 ? (
